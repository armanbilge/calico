...
trait AriaAttrs[F[_]] {
   ...
   */
  protected def ariaAttr[V](key: String, codec: Codec[V, String]): AriaAttr[F, V] =
    new AriaAttr(key, codec)

  @inline protected def boolAsTrueFalseAriaAttr(key: String): AriaAttr[F, Boolean] =
    ariaAttr(key, BooleanAsTrueFalseStringCodec)

  @inline protected def doubleAriaAttr(key: String): AriaAttr[F, Double] =
    ariaAttr(key, DoubleAsStringCodec)

  @inline protected def intAriaAttr(key: String): AriaAttr[F, Int] =
    ariaAttr(key, IntAsStringCodec)

  @inline protected def stringAriaAttr(key: String): AriaAttr[F, String] =
    ariaAttr(key, StringAsIsCodec)
   ...
   * current element. See related [[owns]].
   *
   * @see
   ...
   * operable. See related [[hidden]] and [[readOnly]].
   *
   * @see
   ...
   * users, including whether a pop-up menu of choices is provided by the application.
   * Typically, drop effect functions can only be provided once an object has been grabbed for a
   * drag operation as the drop effect functions available are dependent on the object being
   * dragged.
   *
   ...
   * user as implemented by the author. See related [[disabled]].
   *
   * @see
  ...
  /**
   * Identifies the element (or elements) that labels the current element. See related [[label]]
   * and [[describedBy]].
   *
   * @see
   ...
   * represent the relationship. See related [[controls]].
   *
   * @see
   ...
   * required if all elements in the set are present in the DOM. See related [[setSize]].
   *
   * @see
  ...
  /**
   * Indicates the current "pressed" state of toggle buttons. See related [[checked]] and
   * [[selected]].
   *
   * Enumerated: Tristate – "true" | "false" | "mixed" | undefined (default)
   ...
   * technologies will receive within a live region. See related [[atomic]].
   *
  ...
  /**
   * Indicates the current "selected" state of various widgets. See related [[checked]] and
   * [[pressed]].
   *
   * @see
   ...
   * all elements in the set are present in the DOM. See related [[posInSet]].
   *
   * @see
